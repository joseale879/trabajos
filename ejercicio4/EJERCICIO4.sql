...
CREATE TABLE Libro (
    ...
    FOREIGN KEY (id_categoria) REFERENCES Categor�a(id_categoria)
);
...
UPDATE Usuario SET nombre = 'Mar�a L. L�pez' WHERE id_usuario = 1;
UPDATE Editorial SET nombre = 'Grupo Planeta' WHERE id_editorial = 1;
UPDATE Categor�a SET nombre = 'Ciencia Ficci�n' WHERE id_categoria = 1;
UPDATE Libro SET titulo = 'Cien a�os de soledad ' WHERE id_libro = 1;
UPDATE Autor SET nombre = 'G. G. M�rquez' WHERE id_autor = 1;
...
ALTER TABLE Usuario ADD correo NVARCHAR(100);
ALTER TABLE Editorial ADD pais NVARCHAR(100);
ALTER TABLE Categor�a ADD descripcion NVARCHAR(255);
ALTER TABLE Libro ADD anio_publicacion INT;
ALTER TABLE Autor ADD nacionalidad NVARCHAR(100);

-- 6. ALTER TABLE ... DROP COLUMN

ALTER TABLE Usuario DROP COLUMN correo;
ALTER TABLE Libro DROP COLUMN anio_publicacion;
...
SELECT REPLACE(nombre, 'Mar�a', 'M.') FROM Usuario;
SELECT CHARINDEX('de', titulo) AS posicion FROM Libro;
SELECT CONCAT(nombre, ' (Editorial)') FROM Editorial;

-- 9. SUBCONSULTAS (SELECT ANIDADOS)

SELECT nombre FROM Usuario WHERE id_usuario IN (SELECT id_usuario FROM Reserva);
SELECT titulo FROM Libro WHERE id_libro IN (SELECT id_libro FROM Reserva_Libro);
SELECT nombre FROM Autor WHERE id_autor IN (
  SELECT id_autor FROM Libro_Autor WHERE id_libro IN (
    SELECT id_libro FROM Libro WHERE id_categoria = 2)
);
SELECT nombre FROM Editorial WHERE id_editorial IN (
  SELECT id_editorial FROM Libro WHERE id_libro IN (
    SELECT id_libro FROM Reserva_Libro)
);
SELECT nombre FROM Categor�a WHERE id_categoria IN (
...
FROM Libro L
JOIN Categor�a C ON L.id_categoria = C.id_categoria;
...
AS
BEGIN
    INSERT INTO Reserva (id_usuario, fecha_reserva)
